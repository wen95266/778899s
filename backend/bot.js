...
// 核心状态机
// Phase 1: 基础模型 (5小时 且 1000万次)
// Phase 2: 深度模型 (5小时 且 +1000万次)
let CALC_TASK = {
    isRunning: false,
    phase: 1, 
    startTime: 0,
...
function formatPredictionText(issue, pred, isFinalOrTitle = false) {
    ...
    bot.use(async (ctx, next) => {
        if (ctx.channelPost) {
            if (CHANNEL_ID && isFinalOrTitle === 'string') {
...
🌊 **波色定位**
主：${waveMap[pred.zhu_bo]} | 防：${waveMap[pred.fang_ String(ctx.chat.id) === String(CHANNEL_ID)) return next();
            return;
        }
    ...
    const sendPredictionMsg = async (ctx, isEdit = false) => {
        try {
            ...
            if (isEdit) { await ctx.editMessageText(text, extra).catch(()=>{}); await ctx.answerCbQuery('已刷新'); }bo]}
    ...
    const handleDeepCalc = async (ctx, isRefresh = false) => {
        try {
            ...
            if (CALC_TASK.isRunning && CALC_TASK.phase === 2 && CALC_TASK.currentIssue == row.issue) {
                const now = Date.now();
                const timePct = Math.min(100, Math.floor(((now -console.log(`[计算完成] 第 ${CALC_TASK.currentIssue} 期 (Phase ${CALC_TASK.phase}) - 最终迭代: ${CALC_TASK.iterations}`);
            ...
            try {
                ...
                if (CALC_TASK.phase === 1) {
                    ...
                    if (AUTO_SEND_ENABLED && CHANNEL_ID && CALC_TASK.bestPrediction) {
                        ...
                        bot.telegram CALC_TASK.startTime) / CALC_TASK.targetDuration) * 100));
...
`.trim();
                const extra = { 
                    parse.sendMessage(ADMIN_ID, `✅ 第 ${nextIssue} 期 (基础版) 计算完毕 (迭代${CALC_TASK.iterations}次)，已自动推送。`);
                    }
                } 
                // Phase 2 完成
                ...
                return isRefresh ? ctx.editMessage
            } catch (e) { console.error('任务完成处理失败:', e); }
            return;
        }
        ...
        try {
            if (!CALC_TASK.historyCache) {
                const [rows]Text(text, extra).catch(()=>{}) : ctx.reply(text, extra);
            }
            ...
            if (CALC_TASK.isRunning && CALC_TASK.phase === 1) {
                 const msg = '⚠️ **基础计算尚未完成**\n = await db.query('SELECT numbers, special_code, shengxiao FROM lottery_results ORDER BY issue DESC LIMIT 50');
                CALC_TASK.historyCache = rows;
            }
            ...
            for(let i=0; i<50; i++) {
                ...
                if (score > CALC_TASK.bestScore) {
                    ...
                    if (i === 49) { 
                        if (CALC_TASK.phase === 1) await db.execute('UPDATE lottery_results SET next_prediction=? WHERE issue=?', [JSON.stringify(tempPred), CALC_TASK.currentIssue]);
                        else if (CALC_TASK.phase === 2) await db.execute('UPDATE lottery_results SET deep_prediction=? WHERE issue=?', [JSON.stringify(tempPred), CALC_TASK.currentIssue]);
                    }
                }
                CALC_TASK.iterations++;
            }
        } catch (e) { console.error请等待基础任务达标后再启动。';
            ...
            CALC_TASK = {
                ...
                targetIterations: baseIterations + 10000000, // +1000("计算出错:", e); }
    }, 100); 
    ...
    bot.start((ctx) => {
        ...
        ctx.reply('🤖 智能预测系统 V6.0 (1万次
            ...
            const startMsg = `🚀 **深度计算已启动**\n\n🎯 目标：${nextIssue} 期\2码增强版) 已就绪', getMainMenu());
    });
    ...
    const sendPredictionMsg = async (ctx, isEdit = false) => {
        try {
            ...
            const isCalculatingn⏱️ 最少耗时：5 小时\n🧠 目标增量：+10,000,000 次迭代\n\n后台正在全力计算中...`;
    ...
    bot.action('peek_deep', async (ctx) => {
        if (!CALC_TASK = CALC_TASK.isRunning && CALC_TASK.phase === 1 && CALC_TASK.currentIssue == row.issue;
            
            const text = formatPredictionText(nextIssue, pred, !is.isRunning || !CALC_TASK.bestPrediction) return ctx.answerCbQuery('无数据');
        const nextIssue = parseInt(CALC_TASK.currentIssue) + 1;
    ...
    const sendProgressMsg = async (ctx, isEdit = false) => {
        ...
        const now = Date.now();
        const timePct = Math.min(100, Math.floor(((now - CALC_TASK.startTime) / CALC_TASK.targetDurationCalculating);
            const extra = {
                parse_mode: 'Markdown',
                ...Markup.inlineKeyboard([Markup.button.callback('🔄 刷新数据', 'refresh_pred')])
            };
...
⚡  Date.now();
                ...
                const iterPct = Math.min(100, Math.floor((CALC_TASK.iterations / CALC_TASK.targetIterations) * 100));
                const timeLeft = Math.ceil((CALC_TASK.targetDuration - (now - CALC_TASK.startTime)) / 1000 / 阶段：${phaseName}
...
🎯 均达 100% 任务方可完成_
`.trim();
        const extra = { parse目标：${nextIssue} 期
🔄 当前迭代：${CALC_TASK.iterations} / ${CALC_TASK.targetIterations}
⏱️ 时间进度：${timePct}% (剩 ${timeLeft > 0 ? timeLeft : 0} 分)
🧠 算力进度：${iterPct}%
━━━━━━━━━━━━━━
_需满足 5 小时且 1000 万次迭代方可完成_
`.trim();
                const extra = { 
                    parse_mode: 'Markdown', 
                    ...Markup.inlineKeyboard([
                        _mode: 'Markdown', ...Markup.inlineKeyboard([Markup.button.callback('🔄 刷新', 'refresh_prog')]) };
    ...
    bot.hears(/自动推送/, (ctx) => {
        ...
        ctx.reply(`自动推送: ${AUTO_SEND_ENABLED ? '✅ 开' : '[Markup.button.callback('👁️ 偷看结果', 'peek_deep')],
                        [Markup.button.callback('🔄 刷新进度', 'refresh_deep')]
                    ]) 
                };
    ...
    bot_TASK.isRunning && CALC_TASK.phase === 1) {
                 const msg = '⚠️ **基础计算尚未完成**\n请等待基础任务达标 (5小时且1000万次) 后再启动。';
                 return isRefresh ? ctx.answerCbQuery(msg, {show_alert:true}) : ctx.replyWithMarkdown(msg);
            }
    ...
    bot.on(['text', 'channel_post'], async (ctx) => {
        ...
        if (ctx.chat.type === 'private' && userStates[ctx.from.id] === 'WAITING_DELETE_ISSUE') {
            if (text === '取消') { userStates[ctx.from.id] = null; return ctx.reply('已 (row.next_prediction) startPred = typeof row.next_prediction === 'string' ? JSON.parse(row.next_prediction) : row.next_prediction;
            ...
            CALC_TASK = {
                ...
                startTime: Date.now(),
                targetDuration: 5 * 60 * 60 * 1000, // 5小时
                targetIterations: baseIterations + 10000000, // 目标 = 当前 + 1000万次
                currentIssue: row.issue,
                bestScore: -1,
                bestPrediction: startPred,
                iterations: baseIterations,
    ...
    bot.action('peek_deep', async (ctx) => {
        ...
        const nextIssue = parseInt(CALC_TASK.currentIssue) + 1;
        const msg = formatPredictionText(nextIssue, CALC_取消', getMainMenu()); }
        ...
        if (result) {
            ...
            const jsonPred = JSON.stringify(TASK.bestPrediction, '👁️ 偷看 (计算中)');
        await ctx.reply(msg, { parse_mode: 'Markdown' });
    });
...
`.trim();
    ...
    bot.hears(/自动推送/, (ctx) => {
        AUTO_SEND_ENABLED = !AUTO_SEND_ENABLED;
        ctx.reply(`自动推送: ${AUTO_SEND_ENABLED(),
                    targetDuration: 5 * 60 * 60 * 1000, // 5小时
